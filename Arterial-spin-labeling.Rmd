...
author: "Benjamin M. Kandel and Brian B. Avants"
...
We computed the CBF - but how does this subject's measurements relate to those of other subjects?

Let us compare to *normative data* from the pediatric template of brain perfusion paper.

```{r normative}
myfn = tempfile( fileext = '.csv' )
download.file("https://ndownloader.figshare.com/files/1699436", myfn )
normdf = read.csv( myfn )
hist( normdf$CBFMeanFrontal_Inf_Orb_L  )
```


The PTBP paper used `aal` data.

```{r aalMap}
templateAAL = antsImageRead( path.expand( "~/data/antsExamples/NeuroBattery/data/template/PTBP/Labels/PTBP_T1_AAL.nii.gz" ) )
templateBrain = antsImageRead( path.expand( "~/data/antsExamples/NeuroBattery/data/template/PTBP/PTBP_T1_BrainCerebellum.nii.gz" ) )
subjectBrain = t1 * thresholdImage( seg.t1, 1, Inf )
quickReg = antsRegistration( subjectBrain,   templateBrain, 'Affine'  )
aal2pcasl = antsApplyTransforms( cbf$meancbf, templateAAL, 
  c( reg$fwdtransforms, quickReg$fwdtransforms ),
  interpolator = 'genericLabel' )
```


Now we can quantify the CBF in each region.

```{r aalQuant}
data( 'aal' )
aslaalmask = thresholdImage( seg.asl, 2, 2 )
labmat = labels2matrix( aal2pcasl, aslaalmask, 
                        targetLabels =  aal$label_num, missingVal = 0 )
dim( labmat )
cbfvec = cbf$meancbf[ aslaalmask ]
length( cbfvec )
# now compute summary measurements
subjectSumCBF = labmat %*% cbfvec
labmatMean = labmat / rowSums(  labmat  )
subjectMeanCBF = labmatMean %*% cbfvec
subjectSummaryCBF = data.frame( anatomy = aal$label_name, meanCBF =  subjectMeanCBF )
angCBF = subjectSummaryCBF$meanCBF[ subjectSummaryCBF$anatomy == "Angular_R" ]
print( 
  t.test( angCBF -   
               normdf$CBFMeanAngular_R )  )
ggplot( normdf, aes( CBFMeanAngular_R, fill='red' )) + geom_density(alpha = 0.2) + 
  ggtitle( "CBF Values for this subject in angular gyrus relative to population" ) + 
    geom_vline(  aes(xintercept = angCBF  ) )
```


Exercise: We can do something similar for volumetric data.
